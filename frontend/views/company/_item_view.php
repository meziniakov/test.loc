						...
						<div class="list-slide-box" data-name=<?= $model->name ?>>
							<div class="modern-list ml-2">
								...
								<div class="grid-category-thumb">
									<a href="<?= Url::to(['company/view', 'slug' => ($model->slug) ? $model->slug : $model->id]) ?>" class="overlay-cate"><?= Html::img($img->getUrl('358x229'), ['class' => 'img-responsive', 'alt' => $model->slug]) ?></a>
									...
									<div class="property_meta">
										...
										<h4 class="lst-title"><?= Html::a($model->name, ['company/view', 'slug' => ($model->slug) ? $model->slug : $model->id]) ?><span class="veryfied-author"></span></h4>
									</div>
								</div>
								<div class="modern-list-content">
									<div class="listing-cat">
										<?php if (!empty($model->category->slug)) : ?>
											<?= Html::a("<i class='ti-briefcase bg-a'></i>" . $model->category->title, ['company/category', 'slug' => $model->category->slug], ['class' => 'cat-icon cl-1']) ?>
										<?php else : ?>
