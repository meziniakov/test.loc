						...
						<div class="list-slide-box" data-name=<?= $model->title ?>>
							<div class="modern-list ml-2">
								...
								<?php if(isset($model->city) && isset($model->category)):?>
									<a href="<?= Url::to(['event/view', 'category' => $model->category->slug, 'city' => $model->city->url, 'slug' => $model->slug]) ?>" class="overlay-cate"><?= Html::img($model->getImage()->getUrl('358x229'), ['class' => 'img-responsive', 'alt' => $model->imageRico->alt]) ?></a>
								<?php else :?>
