                ...
                <li class="dropdown user user-menu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                        <?php if (Yii::$app->user->identity->userProfile->avatar_path) : ?>
                            <img src="<?php Yii::getAlias('@storageUrl/avatars/' . Yii::$app->user->identity->userProfile->avatar_path) ?>" class="user-image" alt>
                        <?php else: ?>
                    ...
                    <ul class="dropdown-menu">
                        <li class="user-header">
                            <?php if (Yii::$app->user->identity->userProfile->avatar_path) : ?>
                                <img src="<?php Yii::getAlias('@storageUrl/avatars/' . Yii::$app->user->identity->userProfile->avatar_path) ?>" class="img-circle" alt>
                            <?php else: ?>
                                <img src="<?= Yii::$app->homeUrl . '/static/img/default.png' ?>" class="img-circle" alt>
                            <?php endif ?>
                            <p>
                                <?= Yii::$app->user->identity->username ?>
                                <small><?php Yii::t('backend', 'Member since {0, date}', Yii::$app->user->identity->created_at) ?></small>
                            </p>
                        </li>
